...
void rssiMedio(uint32_t &RXpacketCount, int16_t  &PacketRSSI){
  ...
  if(RXpacketCount % 200 == 0){
    ...
    uint16_t  mediaRSSI = somaRSSI/RXpacketCount;

    Serial.println();
    Serial.print("Intensidade total (RSSI)  = ");
    Serial.print(somaRSSI);
    Serial.println(" dBm");
    Serial.print("Intensidadde média (RSSI)  = "); // RSSI -> indicador de intensidade de sinal recebido 
